...
export default function DemoHow({ eyebrow, title, steps }: Props) {
    ...
    () => {
      ...
      mm.add(
        ...
        (ctx) => {
          ...
          gsap.from(root.querySelectorAll("[data-reveal='how-head']"), {
            y: 28,
            opacity: 0,
            filter: "blur(6px)",
            duration: 1.0,
          ...
          root.querySelectorAll<HTMLElement>("[data-reveal='how-step']").forEach((step, i) => {
            gsap.from(step, {
              y: 40,
              opacity: 0,
              filter: "blur(8px)",
