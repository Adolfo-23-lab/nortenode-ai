...
export default function DemoLimits({ eyebrow, title, items }: Props) {
    ...
    () => {
      ...
      mm.add(
        ...
        (ctx) => {
          ...
          gsap.from(root.querySelectorAll("[data-reveal='limits-head']"), {
            y: 24,
            opacity: 0,
            filter: "blur(6px)",
            duration: 0.9,
          ...
          root.querySelectorAll<HTMLElement>("[data-reveal='limits-item']").forEach((item, i) => {
            gsap.from(item, {
              y: 32,
              opacity: 0,
              filter: "blur(8px)",
